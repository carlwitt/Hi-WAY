...
public abstract class StaticScheduler extends WorkflowScheduler {
	...
	public void enqueueResourceRequest(TaskInstance task) {
		...
		if (HiWayConfiguration.verbose)
			WorkflowDriver.Logger.writeToStdout("Added task " + task + " to queue " + node);
	}
	...
	public TaskInstance scheduleTaskToContainer(Container container) {
		...
		if (HiWayConfiguration.verbose)
			WorkflowDriver.Logger.writeToStdout("Looking for task on container " + container.getId() + " on node " + node + "; Queue:" + readyTasksByNode.get(node).toString());
