...
public class HEFT extends StaticScheduler {
	...
	public HEFT(String workflowName, int containers) {
        ...
        WorkflowDriver.Logger.writeToStdout("HEFT has detected " + containers + " containers per worker node.");
	...
	protected void addTask(TaskInstance task) {
		...
		for (String node : nodes) {
			...
			double computationCost = provenanceManager.runtimeEstimatesPerNode.get(node).get(task.getTaskId()).weight;
		...
		if (HiWayConfiguration.verbose)
            WorkflowDriver.Logger.writeToStdout("Task " + task + " scheduled on node " + bestNode);
	...
	public void addTasks(Collection<TaskInstance> tasks) {
		if (readyTasksByNode.size() == 0) {
            WorkflowDriver.Logger.writeToStdout("No provenance data available for static scheduling. Aborting.");
			System.exit(-1);
		}
		...
		for (int i = taskList.size() - 1; i >= 0; i--) {
			...
			for (String node : nodes) {
				averageComputationCost += provenanceManager.runtimeEstimatesPerNode.get(node).get(task.getTaskId()).weight;
			}
		...
		for (String node : taskOnsetsPerNode.keySet()) {
			...
			for (int i = 0; i < containers; i++) {
				...
				for (Double onset : taskOnsets.keySet()) {
					...
					sb.append("-");
					sb.append(onset + provenanceManager.runtimeEstimatesPerNode.get(node).get(task.getTaskId()).weight);
					sb.append(" ");
				}
				sb.append("\n");
			}
